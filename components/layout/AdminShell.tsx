        ...
        <nav className="flex flex-1 flex-col gap-1 p-3">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded px-3 py-2 font-mono text-xs uppercase tracking-widest text-ivory-50/80 transition-colors hover:bg-navy-800 hover:text-gold-300"
            >
              {item.label}
            </Link>
          ))}
        </nav>
        <form action={signOut} className="border-t border-navy-800 p-3">
          <button
            type="submit"
